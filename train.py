...
def training_step(model, X_batch, y_batch, reg):
    ...
    for param in model.parameters():
        if param.startswith("w"):
            reg_loss += 0.5 * reg * np.sum(model.params[param] ** 2)'''
    reg_loss,gradsw1 = l2_regularization(model.parameters()['weight1'],reg)
    loss += reg_loss
    
    reg_loss,gradsw2 = l2_regularization(model.parameters()['weight2'],reg)
    loss += reg_loss
    
    # Backward pass
    grads = model.backward(dscores, cache)
    grads['weight1'] += gradsw1
    grads['weight2'] += gradsw2
    
    # Add gradient of regularization los
